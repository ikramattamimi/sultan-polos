...
const CardConvection = ({
  convection: { id, name, category, selling_price, stock },
}) => {
  ...
  return (
    <div>
      <div className="card bg-white p-0 min-w-[250px] max-w-1/2">
        <div className="p-4">
          <span className="p-1 rounded-sm bg-gray-200 text-gray-500 text-sm">
            {category}
          </span>
          ...
          <h3 className="dark:text-gray-400">{formatPrice(selling_price)}</h3>
        ...
        <div className="p-4 font-medium text-gray-500">Stock: {stock}</div>
      </div>
    </div>
  );
};
