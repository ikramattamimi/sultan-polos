...
const EditConvectionModal = ({ show, onClose, onUpdate, convectionData }) => {
  ...
  // Load convection data when modal opens or convectionData changes
  useEffect(() => {
    if (show && convectionData) {
      setConvection({
        id: convectionData.id || '',
        name: convectionData.name || '',
        purchase_price: convectionData.purchase_price?.toString() || '',
        stock: convectionData.stock?.toString() || '',
        buffer_stock: convectionData.buffer_stock?.toString() || '',
        unit: convectionData.unit || '',
        color_id: convectionData.color_id || '',
      });
      fetchColors();
    }
  }, [show, convectionData]);
  ...
  const handleSubmit = async () => {
    if (convection.name && convection.purchase_price && convection.stock && convection.buffer_stock && convection.unit) {
      try {
        ...
        console.error('Error updating convection:', err);
        setError('Gagal mengupdate convection. Silakan coba lagi.');
      } finally {
        setLoading(false);
      }
    }
...
export default EditConvectionModal;
